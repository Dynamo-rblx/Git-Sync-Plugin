...
repeat task.wait() until _G.Gitsync
repeat task.wait() until _G.Gitsync.Loaded
---------------------------------------------------

-- FUNCTION DECLARATIONS
----> Disconnect connections
function Functions.Disconnect(target: any) : nil
	for tag, cxn: RBXScriptConnection in _G.Gitsync.Connections do
		if not (tostring(tag) == tostring(target)) then return end
		cxn:Disconnect()
		_G.Gitsync.Connections[tag] = nil
	end
end
...
function Functions.confirm(attempt: string): boolean
	...
	if confirmed == 0 then
		...
		confirm_frame.Warning.TextColor3 = _G.Gitsync.Colors.Red
	else
		confirm_frame.Warning.Text = "Attempt to "..attempt.." confirmed."
		confirm_frame.Warning.TextColor3 = _G.Gitsync.Colors.Green
	end
...
function Functions.getRepoContents(path: string, pullButton: GuiButton)
	...
	if response.Success then
		...
		if pullButton then pullButton.ImageLabel.ImageColor3 = _G.Gitsync.Colors.Red end
		return nil
	end
end
...
function Functions.createEntry(name: string, parentFrame: Frame, isFolder: bool) : TextLabel
	...
	if isFolder then
		...
		entry.template.TextColor3 = _G.Gitsync.Colors.Blue
	end
...
function Functions.populateExplorer(parentFrame: Frame, path: string) : nil
	for i, v in parentFrame:GetChildren() do if not v:IsA("UIListLayout") then v:Destroy() end end
	...
	for _, item in ipairs(contents) do
		...
		if isFolder then
			entry.MouseButton1Click:Connect(function()
				...
				table.insert(_G.Gitsync.ActiveExplorerWidgets, explorer_widget)
	...
	for _, item in pairs(contents) do
		...
		elseif item.type == "file" and (item.name:match("%.lua$") or item.name:match("%.luau$")) then
			...
			if fileData and fileData.content then
				...
				if pullButton then pullButton.ImageLabel.ImageColor3 = _G.Gitsync.Colors.Green end
			end
		end
	end
	return
