...
function Interactions.pushBuild(repo, filePath, content, token)
		local url = "https://api.github.com/repos/" .. repo .. "/contents/" .. filePath

		local requestBody = HttpService:JSONEncode({
			message = "Uploaded model",
			content = Functions.to_base64(content),
		...
		if success then
			print("Successfully uploaded:", filePath)
		else
...
function Interactions.pullBuild(repo, filePath, token)
	local HttpService = game:GetService("HttpService")
	local url = "https://api.github.com/repos/" .. repo .. "/contents/" .. filePath
	...
	if success and response and response.Body then
		...
		if responseData.content then
			local decodedJSON = HttpService:JSONDecode(HttpService:Base64Decode(responseData.content))
			return Functions.deserializeInstance(decodedJSON)
		end
	end
...
function Interactions.pullFromGitHub(repo, token, pullButton)
	...
	--local scriptContainer = game:GetService("ServerScriptService")

	--if #selectedObjects > 0 then
	--	scriptContainer = selectedObjects[1]
	--end
	...
	--		local fileName = file.name:gsub("%.lua$", "")
